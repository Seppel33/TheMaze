...
void main() {
        ...
        // Writefinal color to the framebuffer
          ...
          float attenuation=0;
               ...
               float endIntensity;
                for(int i = 0;i<10;i++){
                flickering=  fs_in.lightIntensity[i];//g(pow(sin(lightflickering+20*i),2)*0.4f+pow(cos(lightflickering*3+20*i),2)*0.5f+pow(sin(8*lightflickering+20*i),2)*0.3f+pow(cos(5*lightflickering+20*i),2)*pow(sin(lightflickering*0.7+20*i),2)+1);
